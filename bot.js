...
const DEFAULT_TEAM = [
  ...
  },
  {
    ...
    scope: ['運營','流量投放','boosting','帳號管理','貼文排程','page 管理','數據分析','執行層面所有事務','團隊分配'],
    isManager: true
  }
];
try {
...
// ── 修正歷史任務：非 Jeff/Joey 的 assignee 一律歸 Joey（他是經理，會自行分配）
try {
  const tasksPath = path.join(DATA_DIR, 'tasks.json');
  if (fs.existsSync(tasksPath)) {
    ...
    for (const t of tasks) {
      ...
      if (!['Jeff', 'Joey'].includes(a)) {
        const before = a || '(空白)';
        t.assignee = 'Joey';
        changed++;
        console.log(`   ${before} → Joey：${t.title}`);
      }
    }
    if (changed > 0) {
      fs.writeFileSync(tasksPath, JSON.stringify(tasks, null, 2));
      console.log(`🔧 修正 ${changed} 項任務 assignee → Joey（經理統籌）`);
    }
  }
} catch (e) {
  console.error('assignee migration error:', e.message);
}
...
- Jeff（老闆）：戰略、業務開發、對外談判、內容方向 — 通常他自己會做
- Joey（運營經理）：所有執行層面事務（運營、流量、帳號、貼文、page 管理、團隊分配等）
⚠️ 公司只有 Jeff 和 Joey 兩人會出現在 assignee。
⚠️ 規則：除了明確是 Jeff 親自要做的戰略/業務開發/對外談判，其餘一律 assignee="Joey"
   Joey 是經理，他自己會分配給下面的人（如 Liqi 等行政人員）

判断规则：
- "記得去問信用卡" → tasks:[{title:"問信用卡",assignee:"Joey"}]
- "設定 page 管理" → tasks:[{title:"設定 page 管理",assignee:"Joey"}]
- "Joey 幫我剪那個 reel" → tasks:[{title:"剪 reel",assignee:"Joey"}]
- "跟 XX 談合作" → tasks:[{title:"跟 XX 談合作",assignee:"Jeff"}]（對外談判）
- "準備 Proposal" → tasks:[{title:"準備 Proposal",assignee:"Jeff"}]（業務開發）
- "明天之前把報價單發出去" → tasks:[{title:"發報價單",assignee:"Joey"}]
...
// ── 9am 團隊任務看板（發到 Operating Team 群，按 assignee 分組 + AI 建議核心 3 件）
async function dailyTeamBoard() {
  try {
    const now = new Date();
    const todayStr = toDateStr(now);
    const tomorrow = new Date(now);
    ...
    const allPendingRaw = getPendingTasks();
    // 群組只關心 Joey 的任務（Jeff 的私下提醒就好）
    const allPending = allPendingRaw.filter(t => {
      const a = (t.assignee || '').trim();
      return a !== 'Jeff'; // Jeff 的排除；其餘（Joey、空白、Chris 等舊資料）都進來
    });

    if (allPending.length === 0) {
      await send(`📋 <b>今日團隊任務看板</b>\n📅 ${todayStr}（${weekday}）\n\n🎉 目前沒有 Joey 需要執行的待辦，可以專注在新的創作上！`, CHAT_ID);
      return;
    }

    const todayOTEvents = await getOTEvents(todayStr, todayStr).catch(() => []);
    const tomorrowOTEvents = await getOTEvents(tomorrowStr, tomorrowStr).catch(() => []);

    // 全部歸 Joey 顯示（Jeff 已過濾掉）
    const byAssignee = { Joey: allPending };
    ...
    const prompt = `你是團隊助理。這是發給 Joey（運營經理）的今日任務看板。Jeff 的個人任務已排除，你只需聚焦 Joey 要執行的事務。請選出 Joey 今天最該完成的 3 件事。

👥 對話對象：Joey（運營經理 — 執行層面所有事務、團隊分配）

📅 今天：${todayStr}（${weekday}）

📋 Joey 的未完成任務（${allPending.length} 項）：
${allPending.map((t, i) => {
  const d = t.date ? ` [${t.date}]` : '';
  const a = t.assignee ? ` [${t.assignee}]` : ' [未指派]';
  const r = t.remindCount > 0 ? ` [拖${t.remindCount}次]` : '';
  return `${i + 1}. ${t.title}${d}${a}${r} (id:${t.id})`;
}).join('\n')}

📅 今日 Operating Team 拍攝/行程：
${todayOTEvents.length > 0 ? todayOTEvents.map(e => `- ${e.start_dt || '全天'} ${e.title}`).join('\n') : '(無)'}
...
2. 直接影響拍攝/出片
3. 已拖延多次
4. 有明確 assignee 的優先（可執行性高）

請嚴格以 JSON 回覆（不要任何額外文字）：
{
  "core3": [
    { "id": "任務id", "rank": 1, "reason": "為什麼今天必做（20字內）", "owner_note": "給負責人的一句話（20字內）" }
  ],
  "team_message": "給 Joey 的一句開場（30字內，直接稱呼 Joey，口吻專業又有溫度）"
}`;
    ...
    // 載入 Joey 的 username 用於 @ 提醒
    let joeyMention = 'Joey';
    try {
      const team = JSON.parse(fs.readFileSync(TEAM_FILE, 'utf-8'));
      const joey = team.find(m => m.name === 'Joey');
      if (joey && joey.telegramUsername) joeyMention = `@${joey.telegramUsername}`;
    } catch {}

    // 組訊息 — 開頭直接 @ Joey
    let msg = `📋 <b>今日團隊任務看板</b>\n📅 ${todayStr}（${weekday}） · ${allPending.length} 項未完成\n${joeyMention} 早安～\n\n`;
    ...
    if (jsonMatch) {
      try {
        ...
        if (plan.team_message) {
          msg += `☀️ ${plan.team_message}\n\n`;
        }

        if (plan.core3 && plan.core3.length > 0) {
          const medals = ['🥇', '🥈', '🥉'];
          msg += `━━━━━━━━━━━━━━━━━━━\n🎯 <b>今日核心 3 件</b>\n\n`;
          plan.core3.forEach((item, i) => {
            const t = taskById[item.id];
            if (!t) return;
            msg += `${medals[i]} <b>${t.title}</b>`;
            if (t.assignee) msg += ` → ${t.assignee}`;
            msg += `\n   💡 ${item.reason}\n`;
            if (item.owner_note) msg += `   📝 ${item.owner_note}\n`;
            msg += '\n';
          });
        }
      } catch (e) {
        console.error('parse core3 error:', e.message);
      }
    }

    // 列出 Joey 的所有任務
    msg += `━━━━━━━━━━━━━━━━━━━\n📦 <b>Joey 全部任務（${allPending.length}）</b>\n\n`;
    msg += allPending.map(t => {
      const d = t.date ? ` <i>(${t.date})</i>` : '';
      const r = t.remindCount > 0 ? ` ⚠️×${t.remindCount}` : '';
      const overdue = t.date && t.date < todayStr ? ' 🔴' : '';
      return `  • ${t.title}${d}${r}${overdue}`;
    }).join('\n');
    ...
    console.error('dailyTeamBoard error:', err.message);
    await send(`❌ 團隊看板生成失敗：${err.message}`, CHAT_ID).catch(() => {});
  }
}
...
async function dailyItemsReminder(label, emoji) {
  const todayStr = toDateStr(new Date());
  // 群組提醒是給下屬的 — 過濾掉 Jeff 的任務（老闆改私人提醒）
  const allPending = getPendingTasks().filter(t => (t.assignee || '').trim() !== 'Jeff');
...
async function dailyEndSummary() {
  ...
  // 今日完成的任務（群組看板：過濾 Jeff，他是老闆不在此提醒）
  const todayTasks = getTasksByDate(todayStr).filter(t => (t.assignee || '').trim() !== 'Jeff');
  ...
  // 未完成的任務（群組看板：過濾 Jeff）
  const allPending6pm = getPendingTasks().filter(t => (t.assignee || '').trim() !== 'Jeff');
  ...
  // 明日待辦任務（群組看板：過濾 Jeff 的任務）
  const tomorrowTasks = getTasksByDate(tomorrowStr)
    .filter(t => t.status !== 'done')
    .filter(t => (t.assignee || '').trim() !== 'Jeff');
...
function scheduleReminders() {
  ...
  setInterval(() => {
    ...
    // ── 9am — 團隊任務看板（發到 Operating Team 群，AI 建議核心 3 件 + 按 assignee 分組）
    //       並私聊 Boss 今日行程
    if (h === 9 && m === 0 && fireKey('9am')) {
      dailyTeamBoard()
        .catch(e => console.error('9am team board error:', e.message));
  ...
  console.log('⏰ 每日提醒：7:30am Top3(私聊) | 9am 團隊看板(群組) | 10/11am 行程 | 2pm 待辦 | 6pm 收工 | 8pm 明日預覽');
  ...
  while (true) {
    try {
      ...
      for (const update of updates) {
        ...
        const isBoardMsg = /^\/?board\b|團隊看板|团队看板|任務看板|任务看板|今日看板/i.test(msgText);
        if (isGroup && !isTaskMsg && !isTeamMsg && !isJeffMsg && !isSummaryMsg && !isQueryMsg && !isDeleteMsg && !isTop3Msg && !isBoardMsg) continue;

        const modeLabel = isBoardMsg ? '[團隊看板]' : isTop3Msg ? '[Top3]' : isJeffMsg ? '[Jeff行程]' : isTeamMsg ? '[拍攝行程]' : isSummaryMsg ? '[待辦總結]' : isQueryMsg ? '[查詢]' : isDeleteMsg ? '[刪除]' : '[任務]';
        ...
        if (isBoardMsg) {
          dailyTeamBoard()
            .catch(e => console.error('Board manual error:', e.message));
          continue;
        }
