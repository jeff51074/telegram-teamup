...
async function dailyTeamBoard() {
  try {
    ...
    const allPending = getPendingTasks();
    if (allPending.length === 0) {
      await send(`📋 <b>今日團隊任務看板</b>\n📅 ${todayStr}（${weekday}）\n\n🎉 目前沒有未完成任務，團隊可以專注在新的創作上！`, CHAT_ID);
      return;
    }
    ...
    // 按 assignee 分組（沒指派的預設歸 Joey — 他是經理負責執行）
    const byAssignee = {};
    for (const t of allPending) {
      let who = t.assignee && t.assignee.trim() ? t.assignee.trim() : 'Joey';
      // 清洗舊資料：Chris/非團隊成員名字一律歸 Joey
      if (!['Jeff', 'Joey'].includes(who)) who = 'Joey';
...
請嚴格以 JSON 回覆（不要任何額外文字）：
{
  ...
  "team_message": "給整個團隊的一句開場（30字內，口吻專業又有溫度）"
}`;
    ...
    // 按 assignee 分組列出所有任務
    msg += `━━━━━━━━━━━━━━━━━━━\n📦 <b>全部任務分配</b>\n\n`;
    const assigneeOrder = Object.keys(byAssignee).sort((a, b) => {
      if (a === '待指派') return 1;
      if (b === '待指派') return -1;
      return byAssignee[b].length - byAssignee[a].length;
    });
    for (const who of assigneeOrder) {
      const tasks = byAssignee[who];
      const icon = who === '待指派' ? '❓' : '👤';
      msg += `${icon} <b>${who}（${tasks.length}）</b>\n`;
      msg += tasks.map(t => {
